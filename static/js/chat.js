document.addEventListener('DOMContentLoaded', function() {
    ...
    function createTrialCard(trial, eligibility, index) {
        ...
        // Use hard_unknown from backend if available, otherwise compute
        const hardUnknown = trial.hard_unknown ?? totalUnknown;
        const hardChecked = eligibility.hardCriteriaMet + (eligibility.hardCriteriaTotal - eligibility.hardCriteriaMet - hardUnknown);

        let criteriaDetail = '';
        if (totalChecked > 0) {
            if (hardUnknown === 0) {
                criteriaDetail = `You meet ${totalMet} of ${totalChecked} checked criteria.`;
            } else if (hardUnknown > hardChecked) {
                criteriaDetail = `${eligibility.hardCriteriaMet} of ${hardChecked} criteria met. ${hardUnknown} more would need clinical verification.`;
            } else {
                criteriaDetail = `You meet ${totalMet} of ${hardChecked + (totalChecked - eligibility.hardCriteriaTotal)} checked criteria. ${hardUnknown} need clinical verification.`;
            }
        }
