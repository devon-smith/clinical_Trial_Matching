...
(declare-const cerebral_ischemic_attack_time_since_most_recent_event_in_months Real) ;; {"when_to_set_to_value":"Set to the number of months since the patient's most recent cerebral ischemic attack, if known.","when_to_set_to_null":"Set to null if the time since the most recent cerebral ischemic attack is unknown, not documented, or cannot be determined.","meaning":"Numeric value indicating the number of months since the patient's most recent cerebral ischemic attack."} ;; "within the past six months"
...
;; Definition: patient has history of cerebral ischemic attack within past 6 months if and only if time since most recent event is less than or equal to 6
(assert
...
(declare-const patient_has_diagnosis_of_malignant_neoplastic_disease_now@@active Bool) ;; {"when_to_set_to_true":"Set to true if the patient's current diagnosis of malignant neoplastic disease (cancer) is active.","when_to_set_to_false":"Set to false if the patient's current diagnosis of malignant neoplastic disease (cancer) is not active.","when_to_set_to_null":"Set to null if it is unknown, not documented, or cannot be determined whether the patient's current diagnosis of malignant neoplastic disease (cancer) is active.","meaning":"Boolean indicating whether the patient's current diagnosis of malignant neoplastic disease (cancer) is active."} ;; "active cancer"

;; ===================== Auxiliary Assertions (REQ 3) =====================
;; Qualifier variable implies corresponding stem variable
(assert
(! (=> patient_has_diagnosis_of_malignant_neoplastic_disease_now@@active
      patient_has_diagnosis_of_malignant_neoplastic_disease_now)
   :named REQ3_AUXILIARY0)) ;; "active cancer" implies "cancer"

;; ===================== Constraint Assertions (REQ 3) =====================
(assert
(! (not patient_has_diagnosis_of_malignant_neoplastic_disease_now@@active)
